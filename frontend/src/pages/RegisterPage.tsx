...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data: tokens } = await axios.post("/api/v1/auth/login", {
        email: form.email,
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-10">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="panel panel-glow p-6">
          <h1 className="mb-1 font-display text-lg font-semibold">Create your AI team</h1>
          <p className="mb-6 text-sm text-ink-muted">
            You'll get four agents ready to go: Personal, Sales, Support, and Opportunity.
          </p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="mb-1.5 block text-xs text-ink-muted">Your name</label>
              <input
                required
            ...
            <div>
              <label className="mb-1.5 block text-xs text-ink-muted">Business name (optional)</label>
              <input
